...
export const BoardSettingsModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/50 p-0 backdrop-blur-sm sm:items-center sm:p-4">
      <section className="w-full max-h-[92vh] overflow-y-auto rounded-t-[28px] border border-white/60 bg-white p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-2xl dark:border-slate-800 dark:bg-slate-900 sm:max-w-3xl sm:rounded-[28px] sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Измени название, колонки и основные действия доски.</p>
            </div>
          </div>
        ...
        <div className="mt-5 space-y-4">
          <div className="rounded-[24px] border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-800/50">
          ...
          <div className="rounded-[24px] border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-800/50">
            <div className="mb-3">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Можно переименовывать и менять местами.</p>
            </div>
            ...
            <div className="space-y-3">
              {columnValues.map((column, index) => (
                <div
                  key={column.id}
                  className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900/70 sm:flex-row sm:items-center"
                >
                  <div className={`h-2 w-full rounded-full bg-gradient-to-r ${column.accent} sm:h-10 sm:w-2 sm:rounded-full`} />
                  <input
                    type="text"
                    value={column.title}
                    onChange={(event) => updateColumnTitle(index, event.target.value)}
                    className="flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-200 dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-100 dark:focus:border-sky-500 dark:focus:ring-sky-500/20"
                  />
                  <div className="flex gap-2 sm:flex-col">
                    <button
                      ...
                      disabled={index === 0}
                      className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 text-slate-600 transition hover:border-sky-300 hover:text-sky-600 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700 dark:text-slate-300 dark:hover:border-sky-500/40 dark:hover:text-sky-300"
          ...
          <div className="rounded-[24px] border border-rose-200 bg-rose-50/70 p-4 dark:border-rose-500/20 dark:bg-rose-500/10">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row">
                  <input
                    ...
                    className="flex-1 rounded-2xl border border-rose-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-rose-400 focus:ring-2 focus:ring-rose-200 dark:border-rose-500/30 dark:bg-slate-950/70 dark:text-slate-100 dark:focus:border-rose-400 dark:focus:ring-rose-500/20"
                  />
                  <button
                    type="button"
                    onClick={onClearAll}
        ...
        <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <button
            ...
            disabled={!canSave}
            className="w-full rounded-2xl bg-slate-900 px-4 py-3 text-sm font-medium text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-sky-500 dark:text-slate-950 dark:hover:bg-sky-400 sm:w-auto"
          >
            Сохранить
          </button>
        </div>
